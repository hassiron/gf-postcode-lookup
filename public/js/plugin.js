...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__Exceptions_PostcodeException__ = __webpack_require__(0);
/**
...
var Postcode = __webpack_require__(4);
 ...
 */
var dollar = function dollar(selector) {
 ...
 */
$(function () {
    var $trigger = dollar('#trigger-lookup');
    var $searchWrap = dollar('.initial');
    var $postcodeInput = $searchWrap.find($trigger.data('postcode-input'));
    var $resultDropdown = $searchWrap.find('.lookup-results');
    var $resultList = $resultDropdown.children('.result-list');
    var $addressFields = dollar('.address-fields');
    var $formToggle = $('#toggle-form-state');
    var $fillableField = dollar('.fillable');
    ...
    if ($trigger.length) {
        $trigger.on('click', function (event) {
            ...
            try {
                var userPostcode = validatePostcode($postcodeInput.val());
                var request = $.ajax({
                    url: AJAX_URL,
                    method: 'POST',
                    data: {
                ...
                request.success(function (response) {
                    ...
                    if (response.status === 200 && response.data.length > 0) {
                        for (var i in response.data) {
                            var address = Object.filter(response.data[i], function (line) {
                                return line.trim() !== '';
                            });
                            var subLines = [];
                            var $el = $('<li />', {
                        ...
                        throw new __WEBPACK_IMPORTED_MODULE_0__Exceptions_PostcodeException__["a" /* default */]("We couldn't find any addresses matching the given postcode");
                    }
                });

                request.fail(function (error, message) {
                    throw new __WEBPACK_IMPORTED_MODULE_0__Exceptions_PostcodeException__["a" /* default */](error);
                });
            } catch (e) {
                $searchWrap.removeClass('loading');
        ...
        $(document).on('click', '.result-list .result', function () {
            ...
            for (var line in lines) {
                if (line in fieldAssociations) {
                    var $input = $('#' + $addressFields.attr('id') + '_' + fieldAssociations[line]);
         ...
         */
        var $addressFieldInputs = $addressFields.find('input');
 ...
 */
var handleSuccessfulLookup = function handleSuccessfulLookup(data) {
    console.log(data);
};
 ...
 */
var validatePostcode = function validatePostcode(postcode) {
    postcode = new Postcode(postcode);

    if (!postcode.valid()) {
        throw new __WEBPACK_IMPORTED_MODULE_0__Exceptions_PostcodeException__["a" /* default */]('The postcode provided is not valid');
    }
